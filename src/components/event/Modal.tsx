...
const Modal: React.FC<props> = ({ eventId }) => {
  ...
  return (
    <>
      ...
      <label htmlFor="my_modal_7" className="btn w-48 xxs:w-60">
        View Results
      </label>
      ...
      <div className="modal" role="dialog">
        <div className="lg:w-1/4 modal-box p-0 overflow-hidden h-[80vh] bg-white">
              ...
              {top3Teams[1] && (
                <div className="flex mt-2 w-1/2 items-center gap-4 flex-col">
                  <div className="w-14 h-14 mt-3 -rotate-6 border-[4px] rounded-xl border-[#979595]">
					<img src={top3Teams[1]?.teamLogo} className="img rounded-lg" alt="" />
                    <div className="flex justify-center items-center">
                      <Image
                        src={silver_badge}
                        alt="First_Badge"
                        className="absolute w-10 top-[20.8em] lg:top-[2em]"
                      />
                    </div>
                  </div>
                  <div className="text-lg line-clamp-1 text-center">{top3Teams[1]?.teamName}</div>
                </div>
              )}
              {top3Teams[0] && (
                <div className="flex w-1/2 items-center gap-4 flex-col">
				  ...
				  <img src={top3Teams[0]?.teamLogo} className="img rounded-lg" alt="" />
                    <div className="flex justify-center items-center">
                      <Image
                        src={first_badge}
                        alt="First_Badge"
                  ...
                  <div className="text-lg line-clamp-1 text-center">{top3Teams[0]?.teamName}</div>
                </div>
              )}
              {top3Teams[2] && (
                <div className="flex mt-2 w-1/2 items-center gap-4 flex-col">
                  <div className="w-14 h-14 mt-3 rotate-6 border-[4px] rounded-xl border-[#9c6a05]">
				  <img src={top3Teams[2]?.teamLogo} className="img rounded-lg" alt="" />
                    <div className="flex justify-center items-center">
                      <Image
                        src={bronze_badge}
                        alt="First_Badge"
                        className="absolute w-12 top-[20.8em] lg:top-[2em]"
                      />
                    </div>
                  </div>
                  <div className=" text-lg text-center line-clamp-1 ">{top3Teams[2]?.teamName}</div>
                </div>
              )}
            </div>
              ...
              {otherTeams &&
                otherTeams.map((data, index) => (
                  ...
                  >
                    ...
                    <div className="pr-3 font-medium">{data.members.length}</div>
                  </div>
                ))}
            </div>
          </div>
